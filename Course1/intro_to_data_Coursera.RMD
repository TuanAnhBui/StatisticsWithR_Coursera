...
sfo_feb_flights %>% summarise(mean_dl = mean(arr_delay),median_dl = median(arr_delay),max_dl = max(arr_delay),earlier = sum(arr_delay<=0),two_hrs_late = sum(arr_delay>=120) )
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(median_dl = median(arr_delay),IQR_dl = IQR(arr_delay))
...
nycflights%>%group_by(month)%>% summarise(mean_dl = mean(dep_delay)) %>% arrange(desc(mean_dl))
```
...
nycflights<-nycflights%>%mutate(avg_speed = distance*60/air_time) %>% arrange(desc(avg_speed))
(nycflights$tailnum)[1]
```
...
nycflights%>% filter(dep_type == "delayed")%>%summarise(prop = sum(arr_type=="on time")/n())
